...
- Emit structured Hawkes sources on every forecast path.
...
ENABLE_NUMINOUS_SIGNALS = str(os.getenv("ENABLE_NUMINOUS_SIGNALS", "1")).strip().lower() in {
    "1",
    "true",
    "yes",
}
...
def normalize_source_timestamp(raw_ts: Any, cutoff: str) -> str:
    now = datetime.now(timezone.utc)
    dt = _parse_iso_datetime(raw_ts)
    if dt is None:
        cutoff_dt = _parse_iso_datetime(cutoff)
        if cutoff_dt is not None:
            dt = min(now - timedelta(minutes=20), cutoff_dt - timedelta(hours=2))
        else:
            dt = now - timedelta(hours=2)
    dt = dt.astimezone(timezone.utc)
    max_age = now - timedelta(hours=SOURCE_MAX_AGE_HOURS - 0.2)
    if dt < max_age:
        dt = max_age
    if dt > now:
...
def classify_source_type(url: str) -> str:
    u = url.lower()
    if "polymarket.com" in u or "gamma-api.polymarket.com" in u:
        return "market_data"
    if "x.com" in u or "twitter.com" in u or "reddit.com" in u:
        return "social_post"
...
def classify_impact_bucket(title: str, snippet: str, source_type: str, domain: str) -> str:
    text = f"{title} {snippet}".lower()
    if any(k in text for k in ("invasion", "default", "bankruptcy", "ceasefire", "emergency", "tariff")):
        return "extreme"
    if any(k in text for k in ("fed", "central bank", "earnings", "guidance", "sanction", "policy", "attack")):
        return "high"
    if source_type == "social_post":
        return "low"
    if domain in ("crypto", "geopolitics") and any(k in text for k in ("regulator", "minister", "official")):
        return "high"
    return "medium"


def classify_persistence_bucket(title: str, snippet: str, domain: str) -> str:
    text = f"{title} {snippet}".lower()
    if any(k in text for k in ("flash", "breaking", "rumor", "tweet", "headline")):
        return "flash"
    if any(k in text for k in ("meeting today", "hours", "today", "tomorrow")):
        return "short"
    if any(k in text for k in ("policy", "law", "sanction", "quarter", "earnings", "guidance")):
        return "medium"
    if domain in ("geopolitics", "weather"):
        return "long"
    return "medium"


def infer_source_direction(prediction: float, prior: float | None) -> str:
    anchor = 0.50 if prior is None else prior
    delta = prediction - anchor
    if delta > 0.03:
        return "up"
    if delta < -0.03:
        return "down"
...
def dedupe_structured_sources(items: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for item in items:
        ...
        seen.add(url)
        out.append(item)
        if len(out) >= MAX_SOURCES_PER_RUN:
            break
    return out
...
def build_structured_sources(
    *,
    ...
    direction = infer_source_direction(prediction, supervisor_prior)
    out: list[dict[str, Any]] = []
    for row in rows:
        url = normalize_url(row.get("url") or row.get("link") or row.get("source_url"))
        if not url:
            continue
        source_title = compact_text(str(row.get("title", "")).strip(), 150)
        snippet = compact_text(str(row.get("snippet", "")).strip(), 260)
        source_type = classify_source_type(url)
        impact_bucket = classify_impact_bucket(source_title, snippet, source_type, domain)
        persistence_bucket = classify_persistence_bucket(source_title, snippet, domain)
        reasoning = compact_text(
            f"Source supports event calibration: {source_title or 'untitled'}; "
            f"snippet={snippet or 'n/a'}. "
            f"Mapped to impact={impact_bucket} and persistence={persistence_bucket} for {domain} event.",
            600,
        )
        out.append(
            {
    ...
    deduped = dedupe_structured_sources(out)
    if len(deduped) >= 3:
        return deduped[:MAX_SOURCES_PER_RUN]

    fallback_ts = normalize_source_timestamp(None, cutoff)
    fallback_sources: list[dict[str, Any]] = [
        {
            "url": f"{POLYMARKET_BASE}/public-search?q={keyword_query(title)}",
            "source_type": "market_data",
            "direction": direction,
            "source_timestamp": fallback_ts,
            "impact_bucket": "medium",
            "persistence_bucket": "medium",
            "reasoning": compact_text(
                "Polymarket public-search context used as market baseline for event pricing and crowd view.",
                280,
            ),
        },
        {
            "url": f"https://www.google.com/search?q={title[:120].replace(' ', '+')}",
            "source_type": "news_article",
            "direction": direction,
            "source_timestamp": fallback_ts,
            "impact_bucket": "low",
            "persistence_bucket": "flash",
            "reasoning": compact_text(
                "Fallback headline scan URL for event-specific recency checks when direct source URLs are sparse.",
                280,
            ),
        },
        {
            ...
            "source_timestamp": fallback_ts,
            "impact_bucket": "low",
            "persistence_bucket": "short",
            "reasoning": compact_text(
                "Reuters used as high-reliability baseline source family for event corroboration fallback.",
                280,
            ),
        },
    ]
    return dedupe_structured_sources(deduped + fallback_sources)[:MAX_SOURCES_PER_RUN]


async def gather_source_rows(
    ...
    *,
    ...
    # Market context source (always useful, especially on exact/related routes).
    rows.append(
        {
            "url": f"{POLYMARKET_BASE}/public-search?q={keyword_query(title)}",
            "title": match.question if match is not None else title,
            "snippet": (
                f"Market anchor yes_price={match.yes_price:.3f} sim={match.similarity:.2f}"
                if match is not None
                else "Polymarket baseline context."
            ),
            "source_timestamp": cutoff,
        }
    )

    # Signals endpoint references when those notes indicate usage.
    joined_notes = " ".join(evidence.notes).lower()
    if "signals:causal_drivers_used" in joined_notes:
        rows.append(
            {
                "url": "https://github.com/numinouslabs/numinous/blob/main/docs/gateway-guide.md#post-apigatewaynuminous-signalscausal-driversdrivers",
                "title": "Numinous Signals causal drivers",
                "snippet": "Causal graph links used for event dependency reasoning.",
                "source_timestamp": cutoff,
            }
        )
    if "signals:deep_research_used" in joined_notes:
        rows.append(
            {
                "url": "https://github.com/numinouslabs/numinous/blob/main/docs/gateway-guide.md#post-apigatewaynuminous-signalsdeep-researchreport",
                "title": "Numinous Signals deep research",
                "snippet": "Precomputed storyline analysis used for event-specific context.",
                "source_timestamp": cutoff,
            }
        )

    # Pull real URLs from Desearch for Hawkes scoring.
    if budget.has(8):
        queries = [title[:96], f"{title[:72]} latest developments"]
        for q in queries:
                ...
                rows.append(
                    {
                        ...
                        "snippet": str(row.get("snippet", "") or row.get("text", ""))[:320],
    ...
    # Lightweight fallback row to keep coverage broad if search is sparse.
    if domain in ("geopolitics", "general"):
        rows.append(
            {
                "url": "https://www.reuters.com/",
                "title": "Reuters top page",
                "snippet": "Fallback high-reliability publication family reference.",
                "source_timestamp": cutoff,
            }
        )
...
async def build_agent_output(
    ...
    *,
    ...
    source_rows = await gather_source_rows(
        client,
        title=title,
    ...
    structured_sources = build_structured_sources(
        title=title,
        domain=domain,
        cutoff=cutoff,
...
def render_reasoning(
    *,
    ...
    evidence: EvidenceBundle,
    llm_reasoning: str,
    structured_sources: list[dict[str, Any]] | None = None,
) -> str:
    evidence_strength = evidence_strength_score(evidence)
    lines: list[str] = []
    lines.append("SOURCES_USED:")
    if structured_sources:
        for idx, src in enumerate(structured_sources[:6], start=1):
            lines.append(
                f"- [S{idx}] {src.get('url', '')} "
                f"(type={src.get('source_type', 'news_article')}, "
                f"impact={src.get('impact_bucket', 'medium')}, "
                f"persistence={src.get('persistence_bucket', 'medium')}, "
                f"ts={src.get('source_timestamp', 'n/a')})"
            )
    elif evidence.sources:
        lines.append(f"- Source families used: {', '.join(evidence.sources[:8])}")
    else:
        lines.append("- Sparse external sources; relied on market structure and fallback controls.")

    lines.append("EVIDENCE_AND_WEIGHTING:")
    market_weight = 0.0
    if match is not None:
        market_weight = 0.70 if match.exact else (0.55 if match.similarity >= 0.88 else 0.42)
    prior_weight = 0.0 if supervisor_prior is None else min(
        0.30,
        PRIOR_WEIGHT_BASE.get("related", 0.10)
        + (1.0 - clamp01(evidence_strength)) * PRIOR_WEIGHT_BOOST_CAP.get("related", 0.12),
    )
    external_weight = clamp01(1.0 - market_weight - prior_weight)
    lines.append(
        f"- Weight split: market={market_weight:.2f}, external={external_weight:.2f}, prior={prior_weight:.2f}."
    )
    if evidence.lines:
        for line in evidence.lines[:3]:
            lines.append(f"- Evidence detail: {line}")
    lines.append(
        f"- Forecast mode={mode}, domain={domain}, bounded to [{PRED_MIN:.2f}, {PRED_MAX:.2f}], evidence_strength={evidence_strength:.2f}."
    )
    if llm_reasoning:
        lines.append(f"- Model rationale summary: {compact_text(llm_reasoning, 420)}")

    lines.append("COUNTERPOINTS_AND_UNCERTAINTY:")
    if match is not None and not match.exact:
        lines.append("- Related-market mismatch risk: similarity not perfect; widened uncertainty band.")
    if supervisor_prior is not None:
        lines.append("- Supervisor prior may contain stale assumptions; adjusted only with evidence-backed shift.")
    lines.append("- Late-breaking updates could invalidate early signals; source recency is explicitly bounded to 36h.")
    ...
    lines.append("PROBABILITY_TRACE:")
    if supervisor_prior is None:
        lines.append(
            f"- No supervisor prior provided; posterior={prediction:.3f} from weighted blend + reliability guard."
        )
    else:
        delta = prediction - supervisor_prior
        lines.append(
            f"- supervisor_prior={supervisor_prior:.3f} -> posterior={prediction:.3f} "
            f"(shift={delta:+.3f}) after evidence weighting and guardrails."
        )
    for note in evidence.notes[:3]:
        lines.append(f"- Note: {note}")

    return "\n".join(lines)[:MAX_REASONING_CHARS]
...
async def forecast(event_data: dict[str, Any]) -> dict[str, Any]:
    ...
    if not title:
        fallback_reasoning = compact_text(
            "SOURCES_USED:\n- Missing title; returned conservative fallback without external evidence.\n"
            "PROBABILITY_TRACE:\n- posterior=0.500 due to invalid event payload.",
            MAX_REASONING_CHARS,
        )
        return {
            "event_id": event_id,
    ...
    async with httpx.AsyncClient(timeout=timeout_cfg) as client:
        ...
        if match.status == "EXACT" and match.exact_match is not None:
            ...
            return await build_agent_output(
                client,
                event_id=event_id,
                title=title,
                description=description,
...
def agent_main(event_data: dict[str, Any]) -> dict[str, Any]:
    try:
        ...
        fallback_reasoning = compact_text(
            "SOURCES_USED:\n- Fatal execution fallback; source extraction unavailable.\n"
            "PROBABILITY_TRACE:\n- posterior=0.500 due to unhandled runtime exception.",
            MAX_REASONING_CHARS,
        )
        return {
            "event_id": str(event_data.get("event_id", "unknown")),
