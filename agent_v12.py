...
def translate_with_prior(posterior: float, prior: float | None, mode: str) -> float:
    if prior is None:
        return clamp(posterior)
    prior_weight = {"exact": 0.06, "related": 0.14, "fallback": 0.20}.get(mode, 0.14)
...
async def build_reasoning_evidence(
    ...
    if domain in ("geopolitics", "crypto", "sports", "general"):
...
async def forecast(event_data: dict[str, Any]) -> dict[str, Any]:
    ...
    async with httpx.AsyncClient(timeout=timeout_cfg) as client:
        ...
        if match.status == "EXACT" and match.exact_match is not None:
            ...
            pred = translate_with_prior(posterior, supervisor_prior, "exact")
        ...
        if domain == "app_store" and match.context:
            ...
            if app_out is not None:
                ...
                pred = translate_with_prior(posterior, supervisor_prior, "related")
                reasoning = render_reasoning(
        ...
        if fallback is not None:
            ...
            posterior = post_calibrate(blended, domain)
            pred = translate_with_prior(posterior, supervisor_prior, "fallback")
            reasoning = render_reasoning(
