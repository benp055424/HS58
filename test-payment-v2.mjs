...
const DRAIN_CONTRACT = '0x0C2B3aA1e80629D572b1f200e6DF3586B3946A8A';
  ...
  for (const prov of providers) {
    ...
    try {
      ...
      const chatRes = await fetch(`${provUrl}/v1/chat/completions`, {
        ...
        // Pick appropriate test payload based on model type
        let testContent = 'Say "Hello from Handshake58 test!" in exactly 5 words.';
        if (modelId.includes('orchestra') || modelId.includes('auto') || modelId.includes('plan') || modelId.includes('pipeline')) {
          testContent = JSON.stringify({ goal: "Say hello in one sentence." });
        } else if (modelId.includes('subnetpulse') || modelId.includes('subnet-brief') || modelId.includes('validator-route') || modelId.includes('miner-route')) {
          testContent = JSON.stringify({ modelHint: "taostats", maxProviders: 3 });
        } else if (modelId.includes('marketintel') || modelId.includes('sector-pulse')) {
          testContent = JSON.stringify({ sector: "AI infrastructure", maxProviders: 3 });
        } else if (modelId.includes('opsguard') || modelId.includes('provider-quote')) {
          testContent = JSON.stringify({ modelHint: "taostats", budgetUsd: 0.10 });
        } else if (modelId.includes('catalogops') || modelId.includes('profile-audit') || modelId.includes('model-coverage') || modelId.includes('launch-readiness')) {
          testContent = JSON.stringify({ providerName: "Mozart-GrowthOps", providerUrl: "https://hs58-production-6250.up.railway.app" });
        } else if (modelId.includes('governanceops') || modelId.includes('policy-check') || modelId.includes('control-matrix') || modelId.includes('release-approval')) {
          testContent = JSON.stringify({ providerName: "Mozart-GrowthOps", providerUrl: "https://hs58-production-6250.up.railway.app" });
        } else if (modelId.includes('qualityops') || modelId.includes('listing-score')) {
          testContent = JSON.stringify({ providerName: "Mozart-GrowthOps", providerUrl: "https://hs58-production-6250.up.railway.app" });
        }
        body: JSON.stringify({
          model: modelId,
