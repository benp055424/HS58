...
export function loadModels(): void {
  ...
  const funnelAuditPrice = parseFloat(optionalEnv('PRICE_FUNNEL_AUDIT', '0.004'));
  const pricingExperimentPrice = parseFloat(optionalEnv('PRICE_PRICING_EXPERIMENT', '0.006'));
  const retentionPlaybookPrice = parseFloat(optionalEnv('PRICE_RETENTION_PLAYBOOK', '0.008'));

  const models: Array<[string, number]> = [
    ['growthops/funnel-audit', funnelAuditPrice],
    ['growthops/pricing-experiment', pricingExperimentPrice],
    ['growthops/retention-playbook', retentionPlaybookPrice],
  ];
  ...
  console.log(`Loaded ${activeModels.size} growthops models`);
}
...
export function loadConfig(): ProviderConfig {
  ...
  return {
    ...
    providerName: optionalEnv('PROVIDER_NAME', 'Mozart-GrowthOps'),
