...
app.get('/v1/pricing', (_req, res) => {
  ...
  for (const id of getSupportedModels()) {
    ...
    models[id] = {
      ...
      outputPer1kTokens: '0',
      description: id === 'growthops/funnel-audit'
        ? 'Audit conversion funnel readiness from discovery to paid agent usage.'
        : id === 'growthops/pricing-experiment'
          ? 'Generate pricing experiment variants and revenue/conversion projections.'
          : 'Build retention playbook to improve repeat usage and reduce churn.',
    };
  }
  ...
  res.json({
    ...
    currency: 'USDC',
    decimals: 6,
    type: 'bittensor-growthops',
    note: 'Conversion and retention growth intelligence for HS58 providers.',
    models,
  });
});
...
app.get('/v1/models', (_req, res) => {
  res.json({
    ...
    data: getSupportedModels().map((id) => ({
      ...
      owned_by: 'mozart-growthops',
    })),
  });
});

app.get('/v1/docs', (_req, res) => {
  const pFunnel = formatUnits(getModelPricing('growthops/funnel-audit')!.inputPer1k, 6);
  const pPricing = formatUnits(getModelPricing('growthops/pricing-experiment')!.inputPer1k, 6);
  const pRetention = formatUnits(getModelPricing('growthops/retention-playbook')!.inputPer1k, 6);

  res.type('text/plain').send(`# Mozart-GrowthOps — Agent Instructions

This is NOT a chat/LLM provider. It returns funnel audit, pricing experiment, and retention playbook outputs using live Handshake58 metadata.

## Models
- growthops/funnel-audit ($${pFunnel})
- growthops/pricing-experiment ($${pPricing})
- growthops/retention-playbook ($${pRetention})
...
- Outputs are strategic recommendations and should be validated before rollout.
`);
});
