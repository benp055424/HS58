...
export function loadModels(): void {
  ...
  const sectorPulsePrice = parseFloat(optionalEnv('PRICE_SECTOR_PULSE', '0.004'));
  const providerGapPrice = parseFloat(optionalEnv('PRICE_PROVIDER_GAP', '0.006'));
  const routeOpportunityPrice = parseFloat(optionalEnv('PRICE_ROUTE_OPPORTUNITY', '0.008'));

  const models: Array<[string, number]> = [
    ['marketintel/sector-pulse', sectorPulsePrice],
    ['marketintel/provider-gap', providerGapPrice],
    ['marketintel/route-opportunity', routeOpportunityPrice],
  ];
  ...
  console.log(`Loaded ${activeModels.size} marketintel models`);
}
...
export function loadConfig(): ProviderConfig {
  ...
  return {
    ...
    providerName: optionalEnv('PROVIDER_NAME', 'Mozart-MarketIntel'),
