...
export function loadModels(): void {
  ...
  const triageBriefPrice = parseFloat(optionalEnv('PRICE_TRIAGE_BRIEF', '0.004'));
  const fallbackSimPrice = parseFloat(optionalEnv('PRICE_FALLBACK_SIM', '0.006'));
  const postmortemDraftPrice = parseFloat(optionalEnv('PRICE_POSTMORTEM_DRAFT', '0.008'));

  const models: Array<[string, number]> = [
    ['incidentops/triage-brief', triageBriefPrice],
    ['incidentops/fallback-sim', fallbackSimPrice],
    ['incidentops/postmortem-draft', postmortemDraftPrice],
  ];
  ...
  console.log(`Loaded ${activeModels.size} incidentops models`);
}
...
export function loadConfig(): ProviderConfig {
  ...
  return {
    ...
    providerName: optionalEnv('PROVIDER_NAME', 'Mozart-IncidentOps'),
