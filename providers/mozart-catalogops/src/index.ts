...
app.get('/v1/pricing', (_req, res) => {
  ...
  for (const id of getSupportedModels()) {
    ...
    models[id] = {
      ...
      outputPer1kTokens: '0',
      description: id === 'catalogops/profile-audit'
        ? 'Audit profile completeness and highlight discovery blockers.'
        : id === 'catalogops/model-coverage'
          ? 'Analyze category/protocol/model coverage and identify catalog blind spots.'
          : 'Generate launch readiness score and action checklist for a candidate provider.',
    };
  }
  ...
  res.json({
    ...
    currency: 'USDC',
    decimals: 6,
    type: 'bittensor-catalogops',
    note: 'Catalog profile quality and launch readiness intelligence for Handshake58 providers.',
    models,
  });
});
...
app.get('/v1/models', (_req, res) => {
  res.json({
    ...
    data: getSupportedModels().map(id => ({
      ...
      owned_by: 'mozart-catalogops',
    })),
  });
});

app.get('/v1/docs', (_req, res) => {
  const pAudit = formatUnits(getModelPricing('catalogops/profile-audit')!.inputPer1k, 6);
  const pCoverage = formatUnits(getModelPricing('catalogops/model-coverage')!.inputPer1k, 6);
  const pReadiness = formatUnits(getModelPricing('catalogops/launch-readiness')!.inputPer1k, 6);
  res.type('text/plain').send(`# Mozart-CatalogOps — Agent Instructions

This is NOT a chat/LLM provider. It returns catalog quality and onboarding readiness intelligence from live Handshake58 provider data.

## Models
- catalogops/profile-audit ($${pAudit})
- catalogops/model-coverage ($${pCoverage})
- catalogops/launch-readiness ($${pReadiness})
...
### catalogops/profile-audit
{
  "providerName": "Mozart-CatalogOps",     // optional exact/partial provider name
  "minCompleteness": 0.8,                  // optional threshold [0..1]
  "tier": "community",                     // optional tier filter
  "marketplaceUrl": "https://handshake58.com" // optional override
}

### catalogops/model-coverage
{
...
### catalogops/launch-readiness
{
  "candidateName": "Mozart-NewProvider",   // required candidate provider name
  "category": "data",                      // optional target category
  "targetProtocol": "drain",               // optional: drain | mpp | x402 | all
  "website": "https://example.com",        // optional, improves readiness score
  "contactEmail": "ops@example.com",       // optional, improves readiness score
  "marketplaceUrl": "https://handshake58.com" // optional override
}

## Response
Assistant content is JSON string with profile audits, coverage stats, and launch checklist recommendations.
...
- Readiness and coverage signals are catalog-driven heuristics and should be validated before deployment.
`);
});
