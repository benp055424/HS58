...
export function loadModels(): void {
  ...
  const profileAuditPrice = parseFloat(optionalEnv('PRICE_PROFILE_AUDIT', '0.004'));
  const modelCoveragePrice = parseFloat(optionalEnv('PRICE_MODEL_COVERAGE', '0.006'));
  const launchReadinessPrice = parseFloat(optionalEnv('PRICE_LAUNCH_READINESS', '0.008'));

  const models: Array<[string, number]> = [
    ['catalogops/profile-audit', profileAuditPrice],
    ['catalogops/model-coverage', modelCoveragePrice],
    ['catalogops/launch-readiness', launchReadinessPrice],
  ];
  ...
  console.log(`Loaded ${activeModels.size} catalogops models`);
}
...
export function loadConfig(): ProviderConfig {
  ...
  return {
    ...
    providerName: optionalEnv('PROVIDER_NAME', 'Mozart-CatalogOps'),
