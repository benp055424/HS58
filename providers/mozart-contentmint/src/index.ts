...
app.get('/v1/pricing', (_req, res) => {
  ...
  for (const id of getSupportedModels()) {
    ...
    models[id] = {
      ...
      outputPer1kTokens: '0',
      description: id === 'contentmint/monetized-research'
        ? 'Research a topic for buyer-intent keywords and monetization potential.'
        : id === 'contentmint/seo-article'
          ? 'Generate deterministic SEO outline and monetization targets.'
          : id === 'contentmint/affiliate-mapper'
            ? 'Rank affiliate opportunities by payout, conversion, and difficulty.'
            : 'Return publication targets and rollout sequence for distribution.',
    };
  }
  ...
  res.json({
    ...
    currency: 'USDC',
    decimals: 6,
    type: 'content-monetization-ops',
    note: 'Deterministic content monetization outputs for research, SEO, affiliates, and distribution.',
    models,
  });
});
...
app.get('/v1/models', (_req, res) => {
  res.json({
    ...
    data: getSupportedModels().map((id) => ({
      ...
      owned_by: 'mozart-contentmint',
    })),
  });
});

app.get('/v1/docs', (_req, res) => {
  const pResearch = formatUnits(getModelPricing('contentmint/monetized-research')!.inputPer1k, 6);
  const pArticle = formatUnits(getModelPricing('contentmint/seo-article')!.inputPer1k, 6);
  const pAffiliate = formatUnits(getModelPricing('contentmint/affiliate-mapper')!.inputPer1k, 6);
  const pTargets = formatUnits(getModelPricing('contentmint/publishing-targets')!.inputPer1k, 6);

  res.type('text/plain').send(`# Mozart-ContentMint — Agent Instructions

This is NOT a chat/LLM provider. It returns deterministic JSON content monetization artifacts.

## Models
- contentmint/monetized-research ($${pResearch})
- contentmint/seo-article ($${pArticle})
- contentmint/affiliate-mapper ($${pAffiliate})
- contentmint/publishing-targets ($${pTargets})

## Input format
Pass one user message with JSON fields:
- topic, audience, buyer_intent, region
- optional source_notes and keyword_seed for monetized-research
- optional angle and monetization_goal_usd_monthly for seo-article
- optional candidate_programs[] for affiliate-mapper
- optional existing_assets[] and publication_count for publishing-targets

## Notes
- Rate limit: ${config.rateLimitPerMinute} req/min per channel
- Outputs are deterministic planning artifacts and should be validated before spending budget.
`);
});
...
${config.providerName} running on http://${config.host}:${config.port}`);
    ...
    console.log(`Content models: ${getSupportedModels().length}`);
