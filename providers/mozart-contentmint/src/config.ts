...
export function loadModels(): void {
  ...
  const researchPrice = parseFloat(optionalEnv('PRICE_MONETIZED_RESEARCH', '0.009'));
  const articlePrice = parseFloat(optionalEnv('PRICE_SEO_ARTICLE', '0.011'));
  const affiliatePrice = parseFloat(optionalEnv('PRICE_AFFILIATE_MAPPER', '0.012'));
  const targetsPrice = parseFloat(optionalEnv('PRICE_PUBLISHING_TARGETS', '0.010'));

  const models: Array<[string, number]> = [
    ['contentmint/monetized-research', researchPrice],
    ['contentmint/seo-article', articlePrice],
    ['contentmint/affiliate-mapper', affiliatePrice],
    ['contentmint/publishing-targets', targetsPrice],
  ];
  ...
  console.log(`Loaded ${activeModels.size} contentmint models`);
}
...
export function loadConfig(): ProviderConfig {
  ...
  return {
    ...
    providerName: optionalEnv('PROVIDER_NAME', 'Mozart-ContentMint'),
