...
export function loadModels(): void {
  ...
  const taskCompilerPrice = parseFloat(optionalEnv('PRICE_TASK_COMPILER', '0.010'));
  const proofBuilderPrice = parseFloat(optionalEnv('PRICE_PROOF_BUILDER', '0.012'));
  const progressAuditorPrice = parseFloat(optionalEnv('PRICE_PROGRESS_AUDITOR', '0.011'));

  const models: Array<[string, number]> = [
    ['execpath/task-compiler', taskCompilerPrice],
    ['execpath/proof-builder', proofBuilderPrice],
    ['execpath/progress-auditor', progressAuditorPrice],
  ];
  ...
  console.log(`Loaded ${activeModels.size} execpath models`);
}
...
export function loadConfig(): ProviderConfig {
  ...
  return {
    ...
    providerName: optionalEnv('PROVIDER_NAME', 'Mozart-Execpath'),
