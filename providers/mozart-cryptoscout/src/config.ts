...
export function loadModels(): void {
  ...
  const scannerPrice = parseFloat(optionalEnv('PRICE_CRYPTO_SCANNER', '0.010'));
  const rankerPrice = parseFloat(optionalEnv('PRICE_OPPORTUNITY_RANKER', '0.012'));
  const sentryPrice = parseFloat(optionalEnv('PRICE_RISK_SENTRY', '0.011'));
  const briefPrice = parseFloat(optionalEnv('PRICE_TRADE_BRIEF', '0.013'));

  const models: Array<[string, number]> = [
    ['cryptoscout/crypto-scanner', scannerPrice],
    ['cryptoscout/opportunity-ranker', rankerPrice],
    ['cryptoscout/risk-sentry', sentryPrice],
    ['cryptoscout/trade-brief', briefPrice],
  ];
  ...
  console.log(`Loaded ${activeModels.size} cryptoscout models`);
}
...
export function loadConfig(): ProviderConfig {
  ...
  return {
    ...
    providerName: optionalEnv('PROVIDER_NAME', 'Mozart-CryptoScout'),
