...
export function loadModels(): void {
  ...
  const serviceReadinessPrice = parseFloat(optionalEnv('PRICE_SERVICE_READINESS', '0.004'));
  const releaseChecklistPrice = parseFloat(optionalEnv('PRICE_RELEASE_CHECKLIST', '0.006'));
  const postLaunchTuningPrice = parseFloat(optionalEnv('PRICE_POST_LAUNCH_TUNING', '0.008'));

  const models: Array<[string, number]> = [
    ['deployops/service-readiness', serviceReadinessPrice],
    ['deployops/release-checklist', releaseChecklistPrice],
    ['deployops/post-launch-tuning', postLaunchTuningPrice],
  ];
  ...
  console.log(`Loaded ${activeModels.size} deployops models`);
}
...
export function loadConfig(): ProviderConfig {
  ...
  return {
    ...
    providerName: optionalEnv('PROVIDER_NAME', 'Mozart-DeployOps'),
