...
export function loadModels(): void {
  ...
  const delegatorProfilePrice = parseFloat(optionalEnv('PRICE_DELEGATOR_PROFILE', '0.004'));
  const campaignPlanPrice = parseFloat(optionalEnv('PRICE_CAMPAIGN_PLAN', '0.006'));
  const retentionPlaybookPrice = parseFloat(optionalEnv('PRICE_RETENTION_PLAYBOOK', '0.008'));

  const models: Array<[string, number]> = [
    ['delegationops/delegator-profile', delegatorProfilePrice],
    ['delegationops/campaign-plan', campaignPlanPrice],
    ['delegationops/retention-playbook', retentionPlaybookPrice],
  ];
  ...
  console.log(`Loaded ${activeModels.size} delegationops models`);
}
...
export function loadConfig(): ProviderConfig {
  ...
  return {
    ...
    providerName: optionalEnv('PROVIDER_NAME', 'Mozart-DelegationOps'),
