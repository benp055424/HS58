...
export function loadModels(): void {
  ...
  const subnetAllocationPrice = parseFloat(optionalEnv('PRICE_SUBNET_ALLOCATION', '0.004'));
  const roleAllocationPrice = parseFloat(optionalEnv('PRICE_ROLE_ALLOCATION', '0.006'));
  const rebalancePlanPrice = parseFloat(optionalEnv('PRICE_REBALANCE_PLAN', '0.008'));

  const models: Array<[string, number]> = [
    ['allocator/subnet-allocation', subnetAllocationPrice],
    ['allocator/role-allocation', roleAllocationPrice],
    ['allocator/rebalance-plan', rebalancePlanPrice],
  ];
  ...
  console.log(`Loaded ${activeModels.size} allocator models`);
}
...
export function loadConfig(): ProviderConfig {
  ...
  return {
    ...
    providerName: optionalEnv('PROVIDER_NAME', 'Mozart-Allocator'),
