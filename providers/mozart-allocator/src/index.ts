...
app.get('/v1/pricing', (_req, res) => {
  ...
  for (const id of getSupportedModels()) {
    ...
    models[id] = {
      ...
      outputPer1kTokens: '0',
      description: id === 'allocator/subnet-allocation'
        ? 'Allocate budget across subnets based on expected opportunity and risk.'
        : id === 'allocator/role-allocation'
          ? 'Split capital between miner and validator roles for target return profile.'
          : 'Generate rebalance plan to improve allocation efficiency over time.',
    };
  }
  ...
  res.json({
    ...
    currency: 'USDC',
    decimals: 6,
    type: 'bittensor-allocator',
    note: 'Capital and role allocation intelligence for Bittensor operators.',
    models,
  });
});
...
app.get('/v1/models', (_req, res) => {
  res.json({
    ...
    data: getSupportedModels().map((id) => ({
      ...
      owned_by: 'mozart-allocator',
    })),
  });
});

app.get('/v1/docs', (_req, res) => {
  const pSubnet = formatUnits(getModelPricing('allocator/subnet-allocation')!.inputPer1k, 6);
  const pRole = formatUnits(getModelPricing('allocator/role-allocation')!.inputPer1k, 6);
  const pRebalance = formatUnits(getModelPricing('allocator/rebalance-plan')!.inputPer1k, 6);

  res.type('text/plain').send(`# Mozart-Allocator — Agent Instructions

This is NOT a chat/LLM provider. It returns deterministic JSON allocation plans for subnet budgets, miner/validator role splits, and rebalance steps.

## How to use via DRAIN
1. Open a payment channel to this provider (drain_open_channel).
2. Call drain_chat with the desired model ID and one user message whose content is a JSON object (see below).

## Models and flat USDC pricing (per request)
- allocator/subnet-allocation — ${pSubnet} USDC — bucket capital across alpha/core/hedge/reserve from risk and horizon
- allocator/role-allocation — ${pRole} USDC — split budget between validator and miner roles
- allocator/rebalance-plan — ${pRebalance} USDC — shift miner vs validator mix toward targets under a drawdown cap

Full numeric rates: GET /v1/pricing

## Input (user message content = JSON)
- allocator/subnet-allocation: optional subnetId, capitalUsd, riskProfile (conservative|balanced|aggressive), horizonDays, minLiquidityScore (0-1).
- allocator/role-allocation: optional subnetId, capitalUsd, opsHoursPerDay, targetRisk (low|medium|high), uptimeTarget (0-1).
- allocator/rebalance-plan: optional subnetId, currentMinerShare, currentValidatorShare, targetDrawdown (0-1), horizonDays, rebalanceBudgetUsd.

## Example
model: "allocator/subnet-allocation"
messages: [{"role":"user","content":"{\\"subnetId\\":13,\\"capitalUsd\\":8000,\\"riskProfile\\":\\"balanced\\",\\"horizonDays\\":45}"}]

## Response
Assistant message content is a JSON string: allocation breakdown, role shares, or rebalance plan with target shares and cadence.

## Notes
- Rate limit: ${config.rateLimitPerMinute} requests/minute per channel.
- Validate outputs against live subnet conditions before acting.
`);
});
