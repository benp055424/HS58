...
app.get('/v1/pricing', (_req, res) => {
  ...
  for (const id of getSupportedModels()) {
    ...
    models[id] = {
      ...
      outputPer1kTokens: '0',
      description: id === 'governanceops/policy-check'
        ? 'Evaluate a target listing against governance policy signals and threshold constraints.'
        : id === 'governanceops/control-matrix'
          ? 'Build a control coverage matrix and highlight systematic governance gaps.'
          : 'Issue release approval decision with readiness score and required actions.',
    };
  }
  ...
  res.json({
    ...
    currency: 'USDC',
    decimals: 6,
    type: 'bittensor-governanceops',
    note: 'Governance and control-assurance intelligence for Handshake58 listings.',
    models,
  });
});
...
app.get('/v1/models', (_req, res) => {
  res.json({
    ...
    data: getSupportedModels().map((id) => ({
      ...
      owned_by: 'mozart-governanceops',
    })),
  });
});

app.get('/v1/docs', (_req, res) => {
  const pPolicy = formatUnits(getModelPricing('governanceops/policy-check')!.inputPer1k, 6);
  const pControl = formatUnits(getModelPricing('governanceops/control-matrix')!.inputPer1k, 6);
  const pRelease = formatUnits(getModelPricing('governanceops/release-approval')!.inputPer1k, 6);

  res.type('text/plain').send(`# Mozart-GovernanceOps — Agent Instructions

This is NOT a chat/LLM provider. It returns governance policy, control coverage, and release-approval intelligence from live Handshake58 provider data.

## Models
- governanceops/policy-check ($${pPolicy})
- governanceops/control-matrix ($${pControl})
- governanceops/release-approval ($${pRelease})
...
### governanceops/policy-check
{
  "providerName": "Mozart",                    // required exact/partial provider name
  "category": "data",                           // optional filter
  "protocol": "drain",                          // optional filter
  "minQualityScore": 60,                         // optional policy threshold
  "maxProviders": 6,                             // optional cap [1..20]
  "marketplaceUrl": "https://handshake58.com"  // optional override
}

### governanceops/control-matrix
{
  "category": "data",                           // optional filter
  "protocol": "all",                            // optional: drain | mpp | x402 | all
  "requiredControls": ["docs_url","api_url"], // optional custom controls
  "limit": 15,                                   // optional cap [1..40]
  "marketplaceUrl": "https://handshake58.com"  // optional override
}

### governanceops/release-approval
{
  "candidateName": "Mozart-NewProvider",       // required candidate label
  "category": "data",                           // optional benchmark category
  "targetProtocol": "drain",                    // optional protocol target
  "minimumReadinessScore": 0.72,                 // optional decision threshold [0..1]
  "minBenchmarkCount": 6,                        // optional [1..30]
  "marketplaceUrl": "https://handshake58.com"  // optional override
}

## Response
Assistant content is JSON string with policy decisions, control matrices, release approval verdicts, and benchmark references.
...
- Governance results are operational guidance and should be validated with final human review.
`);
});
