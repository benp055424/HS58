...
export function loadModels(): void {
  ...
  const policyCheckPrice = parseFloat(optionalEnv('PRICE_POLICY_CHECK', '0.004'));
  const controlMatrixPrice = parseFloat(optionalEnv('PRICE_CONTROL_MATRIX', '0.006'));
  const releaseApprovalPrice = parseFloat(optionalEnv('PRICE_RELEASE_APPROVAL', '0.008'));

  const models: Array<[string, number]> = [
    ['governanceops/policy-check', policyCheckPrice],
    ['governanceops/control-matrix', controlMatrixPrice],
    ['governanceops/release-approval', releaseApprovalPrice],
  ];
  ...
  console.log(`Loaded ${activeModels.size} governanceops models`);
}
...
export function loadConfig(): ProviderConfig {
  ...
  return {
    ...
    providerName: optionalEnv('PROVIDER_NAME', 'Mozart-GovernanceOps'),
