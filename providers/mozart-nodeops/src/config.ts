...
export function loadModels(): void {
  ...
  const minerBootstrapPrice = parseFloat(optionalEnv('PRICE_MINER_BOOTSTRAP', '0.004'));
  const validatorBootstrapPrice = parseFloat(optionalEnv('PRICE_VALIDATOR_BOOTSTRAP', '0.006'));
  const emissionsSimPrice = parseFloat(optionalEnv('PRICE_EMISSIONS_SIM', '0.008'));

  const models: Array<[string, number]> = [
    ['nodeops/miner-bootstrap', minerBootstrapPrice],
    ['nodeops/validator-bootstrap', validatorBootstrapPrice],
    ['nodeops/emissions-sim', emissionsSimPrice],
  ];
  ...
  console.log(`Loaded ${activeModels.size} nodeops models`);
}
...
export function loadConfig(): ProviderConfig {
  ...
  return {
    ...
    providerName: optionalEnv('PROVIDER_NAME', 'Mozart-NodeOps'),
