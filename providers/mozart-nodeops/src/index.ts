...
app.get('/v1/pricing', (_req, res) => {
  ...
  for (const id of getSupportedModels()) {
    ...
    models[id] = {
      ...
      outputPer1kTokens: '0',
      description: id === 'nodeops/miner-bootstrap'
        ? 'Create subnet-specific miner bootstrapping plan with infra and wallet runbook.'
        : id === 'nodeops/validator-bootstrap'
          ? 'Create validator launch and hardening plan with stake, uptime, and monitoring controls.'
          : 'Simulate emissions and profitability under cost, uptime, and share assumptions.',
    };
  }
  ...
  res.json({
    ...
    currency: 'USDC',
    decimals: 6,
    type: 'bittensor-nodeops',
    note: 'Node launch and profitability intelligence for Bittensor subnet miners and validators.',
    models,
  });
});
...
app.get('/v1/models', (_req, res) => {
  res.json({
    ...
    data: getSupportedModels().map((id) => ({
      ...
      owned_by: 'mozart-nodeops',
    })),
  });
});

app.get('/v1/docs', (_req, res) => {
  const pMiner = formatUnits(getModelPricing('nodeops/miner-bootstrap')!.inputPer1k, 6);
  const pValidator = formatUnits(getModelPricing('nodeops/validator-bootstrap')!.inputPer1k, 6);
  const pSim = formatUnits(getModelPricing('nodeops/emissions-sim')!.inputPer1k, 6);

  res.type('text/plain').send(`# Mozart-NodeOps — Agent Instructions

This is NOT a chat/LLM provider. It returns miner/validator bootstrap and emissions simulation outputs to help users launch subnet operations end-to-end.

## Models
- nodeops/miner-bootstrap ($${pMiner})
- nodeops/validator-bootstrap ($${pValidator})
- nodeops/emissions-sim ($${pSim})

## Input format
Pass one user message with JSON fields relevant to each model.

## Notes
- Rate limit: ${config.rateLimitPerMinute} req/min per channel
- Outputs are operational plans and profit simulations and should be validated against live subnet conditions.
`);
});
