...
export function loadModels(): void {
  ...
  const scannerPrice = parseFloat(optionalEnv('PRICE_GIG_SCANNER', '0.009'));
  const drafterPrice = parseFloat(optionalEnv('PRICE_PROPOSAL_DRAFTER', '0.011'));
  const trackerPrice = parseFloat(optionalEnv('PRICE_CONVERSION_TRACKER', '0.010'));
  const plannerPrice = parseFloat(optionalEnv('PRICE_ARBITRAGE_PLANNER', '0.012'));

  const models: Array<[string, number]> = [
    ['gigarb/gig-scanner', scannerPrice],
    ['gigarb/proposal-drafter', drafterPrice],
    ['gigarb/conversion-tracker', trackerPrice],
    ['gigarb/arbitrage-planner', plannerPrice],
  ];
  ...
  console.log(`Loaded ${activeModels.size} gigarb models`);
}
...
export function loadConfig(): ProviderConfig {
  ...
  return {
    ...
    providerName: optionalEnv('PROVIDER_NAME', 'Mozart-GigArb'),
