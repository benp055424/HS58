...
export function loadModels(): void {
  ...
  const subnetBriefPrice = parseFloat(optionalEnv('PRICE_SUBNET_BRIEF', '0.004'));
  const validatorRoutePrice = parseFloat(optionalEnv('PRICE_VALIDATOR_ROUTE', '0.006'));
  const minerRoutePrice = parseFloat(optionalEnv('PRICE_MINER_ROUTE', '0.008'));

  const models: Array<[string, number]> = [
    ['subnetpulse/subnet-brief', subnetBriefPrice],
    ['subnetpulse/validator-route', validatorRoutePrice],
    ['subnetpulse/miner-route', minerRoutePrice],
  ];
  ...
  console.log(`Loaded ${activeModels.size} subnetpulse models`);
}
...
export function loadConfig(): ProviderConfig {
  ...
  return {
    ...
    storagePath: optionalEnv('STORAGE_PATH', '/app/data/vouchers.json'),
    providerName: optionalEnv('PROVIDER_NAME', 'Mozart-Subnetpulse'),
