...
export function loadModels(): void {
  ...
  const quotePrice = parseFloat(optionalEnv('PRICE_PROVIDER_QUOTE', '0.004'));
  const budgetRoutePrice = parseFloat(optionalEnv('PRICE_BUDGET_ROUTE', '0.006'));
  const failoverPrice = parseFloat(optionalEnv('PRICE_FAILOVER_PLAN', '0.008'));

  const models: Array<[string, number]> = [
    ['opsguard/provider-quote', quotePrice],
    ['opsguard/budget-route', budgetRoutePrice],
    ['opsguard/failover-plan', failoverPrice],
  ];
  ...
  console.log(`Loaded ${activeModels.size} opsguard models`);
}
...
export function loadConfig(): ProviderConfig {
  ...
  return {
    ...
    providerName: optionalEnv('PROVIDER_NAME', 'Mozart-Opsguard'),
