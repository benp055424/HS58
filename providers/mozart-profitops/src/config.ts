...
export function loadModels(): void {
  ...
  const opportunityScanPrice = parseFloat(optionalEnv('PRICE_OPPORTUNITY_SCAN', '0.004'));
  const roiForecastPrice = parseFloat(optionalEnv('PRICE_ROI_FORECAST', '0.006'));
  const executionPlaybookPrice = parseFloat(optionalEnv('PRICE_EXECUTION_PLAYBOOK', '0.008'));

  const models: Array<[string, number]> = [
    ['profitops/opportunity-scan', opportunityScanPrice],
    ['profitops/roi-forecast', roiForecastPrice],
    ['profitops/execution-playbook', executionPlaybookPrice],
  ];
  ...
  console.log(`Loaded ${activeModels.size} profitops models`);
}
...
export function loadConfig(): ProviderConfig {
  ...
  return {
    ...
    providerName: optionalEnv('PROVIDER_NAME', 'Mozart-ProfitOps'),
