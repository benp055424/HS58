...
app.get('/v1/pricing', (_req, res) => {
  ...
  for (const id of getSupportedModels()) {
    ...
    models[id] = {
      ...
      outputPer1kTokens: '0',
      description: id === 'profitops/opportunity-scan'
        ? 'Scan highest-value provider opportunities by category, margin, and demand signal.'
        : id === 'profitops/roi-forecast'
          ? 'Forecast ROI for a candidate monetization plan using spend, conversion, and retention inputs.'
          : 'Generate execution playbook to launch and compound provider revenue safely.',
    };
  }
  ...
  res.json({
    ...
    currency: 'USDC',
    decimals: 6,
    type: 'bittensor-profitops',
    note: 'Revenue opportunity and ROI execution intelligence for HS58 providers.',
    models,
  });
});
...
app.get('/v1/models', (_req, res) => {
  res.json({
    ...
    data: getSupportedModels().map((id) => ({
      ...
      owned_by: 'mozart-profitops',
    })),
  });
});

app.get('/v1/docs', (_req, res) => {
  const pOpportunity = formatUnits(getModelPricing('profitops/opportunity-scan')!.inputPer1k, 6);
  const pRoi = formatUnits(getModelPricing('profitops/roi-forecast')!.inputPer1k, 6);
  const pPlaybook = formatUnits(getModelPricing('profitops/execution-playbook')!.inputPer1k, 6);

  res.type('text/plain').send(`# Mozart-ProfitOps — Agent Instructions

This is NOT a chat/LLM provider. It returns opportunity scan, ROI forecast, and execution playbook outputs using live Handshake58 metadata.

## Models
- profitops/opportunity-scan ($${pOpportunity})
- profitops/roi-forecast ($${pRoi})
- profitops/execution-playbook ($${pPlaybook})
...
- Outputs are monetization recommendations and should be validated before rollout.
`);
});
