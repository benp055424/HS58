...
function pricingDescription(id: string): string {
  switch (id) {
    case 'launchpadops/miner-go-live':
      return 'Miner go-live checklist with deterministic readiness score (flat per request).';
    case 'launchpadops/validator-go-live':
      return 'Validator go-live checklist with deterministic readiness score (flat per request).';
    case 'launchpadops/first-30-days':
      return 'First 30 days milestone plan for miner or validator (flat per request).';
    default:
      return 'Launchpad planning output (flat per request).';
  }
}
...
app.get('/v1/pricing', (_req, res) => {
  ...
  res.json({
    ...
    currency: 'USDC',
    decimals: 6,
    type: 'bittensor-launchpadops',
    note: 'Miner/validator go-live checklists and first-30-days playbooks for subnet operators.',
    models,
  });
});
...
app.get('/v1/docs', (_req, res) => {
  const pMiner = formatUnits(getModelPricing('launchpadops/miner-go-live')!.inputPer1k, 6);
  const pVal = formatUnits(getModelPricing('launchpadops/validator-go-live')!.inputPer1k, 6);
  const p30 = formatUnits(getModelPricing('launchpadops/first-30-days')!.inputPer1k, 6);

  res.type('text/plain').send(`# Mozart-LaunchpadOps — Agent Instructions

This is NOT a chat/LLM provider. It returns deterministic JSON for miner/validator go-live readiness and a first-30-days operating plan.

## Models (flat USDC per request — see GET /v1/pricing)
- launchpadops/miner-go-live ($${pMiner})
- launchpadops/validator-go-live ($${pVal})
- launchpadops/first-30-days ($${p30})
...
## Input hints
- miner-go-live: { "subnetId", "walletReady", "axonPortOpen", "registrationComplete", "depsInstalled", "smokeTestPass" } (booleans)
- validator-go-live: { "subnetId", "stakeReady", "hotkeyRegistered", "rpcEndpointsOk", "weightsCommitOk", "failoverPlanReady" }
- first-30-days: { "subnetId", "role": "miner"|"validator", "startEpoch", "timezone" }

## Example
model: "launchpadops/miner-go-live"
messages: [{"role":"user","content":"{\\"subnetId\\":13,\\"registrationComplete\\":true,\\"smokeTestPass\\":true}"}]

## Response
Assistant message content is a JSON string with checklist results, scores, or milestone tables.
...
- Outputs are operational aids; verify against live subnet requirements and your runbooks.
`);
});
