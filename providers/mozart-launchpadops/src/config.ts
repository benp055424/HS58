...
export function loadModels(): void {
  ...
  const minerGoLivePrice = parseFloat(optionalEnv('PRICE_MINER_GO_LIVE', '0.004'));
  const validatorGoLivePrice = parseFloat(optionalEnv('PRICE_VALIDATOR_GO_LIVE', '0.006'));
  const first30DaysPrice = parseFloat(optionalEnv('PRICE_FIRST_30_DAYS', '0.008'));

  const models: Array<[string, number]> = [
    ['launchpadops/miner-go-live', minerGoLivePrice],
    ['launchpadops/validator-go-live', validatorGoLivePrice],
    ['launchpadops/first-30-days', first30DaysPrice],
  ];
  ...
  console.log(`Loaded ${activeModels.size} launchpadops models`);
}
...
export function loadConfig(): ProviderConfig {
  ...
  return {
    ...
    providerName: optionalEnv('PROVIDER_NAME', 'Mozart-LaunchpadOps'),
