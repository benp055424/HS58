...
app.get('/v1/pricing', (_req, res) => {
  ...
  for (const id of getSupportedModels()) {
    ...
    models[id] = {
      ...
      outputPer1kTokens: '0',
      description:
        id === 'mineralpha/task-yield-model'
          ? 'Estimate daily task yield and net margin from throughput, success rate, and per-task economics.'
          : id === 'mineralpha/hardware-roi'
            ? 'Compute hardware payback and horizon ROI from power, uptime, and gross daily revenue assumptions.'
            : 'Suggest worker and batch tuning to reach target RPS with deterministic throughput projections.',
    };
  }
  ...
  res.json({
    ...
    currency: 'USDC',
    decimals: 6,
    type: 'bittensor-mineralpha',
    note: 'Miner-focused yield, hardware ROI, and throughput tuning for subnet operators.',
    models,
  });
});
...
app.get('/v1/models', (_req, res) => {
  res.json({
    ...
    data: getSupportedModels().map((id) => ({
      ...
      owned_by: 'mozart-mineralpha',
    })),
  });
});

app.get('/v1/docs', (_req, res) => {
  const pYield = formatUnits(getModelPricing('mineralpha/task-yield-model')!.inputPer1k, 6);
  const pRoi = formatUnits(getModelPricing('mineralpha/hardware-roi')!.inputPer1k, 6);
  const pTuning = formatUnits(getModelPricing('mineralpha/throughput-tuning')!.inputPer1k, 6);

  res.type('text/plain').send(`# Mozart-MinerAlpha — Agent Instructions

This is NOT a chat/LLM provider. It returns deterministic JSON for miner task yield, hardware ROI, and throughput tuning.

## Models and prices (USDC per request)
- mineralpha/task-yield-model — $${pYield}
- mineralpha/hardware-roi — $${pRoi}
- mineralpha/throughput-tuning — $${pTuning}

## How to use via DRAIN
1. Open a payment channel to this provider (drain_open_channel).
2. Call drain_chat with model set to one of the IDs above and a single user message whose content is JSON (see below).

## Input format (user message = JSON)
- task-yield-model: optional subnetId, role (miner|validator), tasksPerDay, successRate (0-1), rewardUsdPerKTasks, overheadUsdDaily.
- hardware-roi: optional subnetId, hardwareCostUsd, powerWatts, kwhUsd, uptime (0-1), dailyGrossUsd, horizonDays.
- throughput-tuning: optional subnetId, currentRps, targetRps, p99LatencyMs, workerCount, batchSize.

## Example
model: "mineralpha/task-yield-model"
messages: [{"role":"user","content":"{\\"subnetId\\":13,\\"role\\":\\"miner\\",\\"tasksPerDay\\":10000,\\"successRate\\":0.95}"}]

## Response
Assistant message content is a JSON string with computed fields and a generatedAt ISO timestamp.

## Pricing
Flat per request; see GET /v1/pricing for current string amounts (inputPer1kTokens is the price, outputPer1kTokens is 0).

## Notes
- Rate limit: ${config.rateLimitPerMinute} requests/minute per channel.
- Outputs are planning aids; validate against live subnet and hardware data before acting.
`);
});
