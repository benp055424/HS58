...
export function loadModels(): void {
  ...
  const taskYieldPrice = parseFloat(optionalEnv('PRICE_TASK_YIELD_MODEL', '0.004'));
  const hardwareRoiPrice = parseFloat(optionalEnv('PRICE_HARDWARE_ROI', '0.006'));
  const throughputTuningPrice = parseFloat(optionalEnv('PRICE_THROUGHPUT_TUNING', '0.008'));

  const models: Array<[string, number]> = [
    ['mineralpha/task-yield-model', taskYieldPrice],
    ['mineralpha/hardware-roi', hardwareRoiPrice],
    ['mineralpha/throughput-tuning', throughputTuningPrice],
  ];
  ...
  console.log(`Loaded ${activeModels.size} mineralpha models`);
}
...
export function loadConfig(): ProviderConfig {
  ...
  return {
    ...
    providerName: optionalEnv('PROVIDER_NAME', 'Mozart-MinerAlpha'),
