...
export function loadModels(): void {
  ...
  const commissionStrategyPrice = parseFloat(optionalEnv('PRICE_COMMISSION_STRATEGY', '0.004'));
  const delegationPricingPrice = parseFloat(optionalEnv('PRICE_DELEGATION_PRICING', '0.006'));
  const rewardCurvePrice = parseFloat(optionalEnv('PRICE_REWARD_CURVE', '0.008'));

  const models: Array<[string, number]> = [
    ['validatoralpha/commission-strategy', commissionStrategyPrice],
    ['validatoralpha/delegation-pricing', delegationPricingPrice],
    ['validatoralpha/reward-curve', rewardCurvePrice],
  ];
  ...
  console.log(`Loaded ${activeModels.size} validatoralpha models`);
}
...
export function loadConfig(): ProviderConfig {
  ...
  return {
    ...
    providerName: optionalEnv('PROVIDER_NAME', 'Mozart-ValidatorAlpha'),
