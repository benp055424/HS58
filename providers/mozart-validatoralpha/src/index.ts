...
const MODEL_COMMISSION = 'validatoralpha/commission-strategy';
const MODEL_DELEGATION = 'validatoralpha/delegation-pricing';
const MODEL_REWARD = 'validatoralpha/reward-curve';
...
app.get('/v1/pricing', (_req, res) => {
  ...
  for (const id of getSupportedModels()) {
    ...
    models[id] = {
      ...
      description:
        id === MODEL_COMMISSION
          ? 'Commission strategy: suggested take rate, competitiveness vs peers, and validator stake context.'
          : id === MODEL_DELEGATION
            ? 'Delegation pricing: minimum delegate, fee band vs peer median, and attractiveness score.'
            : 'Reward curve: deterministic cumulative TAO projection over epochs with sensitivity bands.',
    };
  }
  ...
  res.json({
    ...
    currency: 'USDC',
    decimals: 6,
    type: 'bittensor-validatoralpha',
    note: 'Validator economics: commission, delegation pricing, and reward projections for subnet operators.',
    models,
  });
});
...
app.get('/v1/models', (_req, res) => {
  res.json({
    ...
    data: getSupportedModels().map((id) => ({
      ...
      owned_by: 'mozart-validatoralpha',
    })),
  });
});

app.get('/v1/docs', (_req, res) => {
  const pCommission = formatUnits(getModelPricing(MODEL_COMMISSION)!.inputPer1k, 6);
  const pDelegation = formatUnits(getModelPricing(MODEL_DELEGATION)!.inputPer1k, 6);
  const pReward = formatUnits(getModelPricing(MODEL_REWARD)!.inputPer1k, 6);

  res.type('text/plain').send(`# Mozart-ValidatorAlpha — Agent Instructions

This is NOT a chat/LLM provider. It returns deterministic JSON for validator commission strategy, delegation pricing, and reward-curve projections.

## Models and default prices (USDC per request)
- ${MODEL_COMMISSION} — $${pCommission}
- ${MODEL_DELEGATION} — $${pDelegation}
- ${MODEL_REWARD} — $${pReward}

## How to use via DRAIN
1. Open a payment channel to this provider (drain_open_channel).
2. Call drain_chat with model set to one of the IDs above.
3. Send one user message whose content is a JSON object; omitted fields use documented defaults inside each tool.

## Input hints
- ${MODEL_COMMISSION}: optional keys subnetId, validatorStakeTao, targetTakeRatePct, competitorAvgCommissionPct, epochsHorizon.
- ${MODEL_DELEGATION}: optional keys subnetId, minDelegationTao, desiredFillRate, peerMedianCommissionPct, operatingCostUsdMonthly.
- ${MODEL_REWARD}: optional keys subnetId, emissionPerEpochTao, delegationMultiple, epochs, baseStakeTao.

## Example
model: "${MODEL_COMMISSION}"
Put JSON in the user message content, e.g. {"subnetId":13,"validatorStakeTao":300,"targetTakeRatePct":16}

## Response
Assistant message content is a JSON string from the tool (commission plan, pricing table, or reward curve sample).

## Pricing
Flat per request per model. Full rates: GET /v1/pricing

## Notes
- Rate limit: ${config.rateLimitPerMinute} requests/minute per channel.
- Outputs are planning aids; validate against live subnet parameters and emissions.
`);
});
