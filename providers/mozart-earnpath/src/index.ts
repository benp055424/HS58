...
app.get('/v1/pricing', (_req, res) => {
  ...
  for (const id of getSupportedModels()) {
    ...
    models[id] = {
      ...
      outputPer1kTokens: '0',
      description: id === 'earnpath/income-map'
        ? 'Generate a personalized Bittensor earning lane map with time-to-revenue and risk profile.'
        : id === 'earnpath/weekly-plan'
          ? 'Convert an earning strategy into week-by-week execution tasks with money-linked artifacts.'
          : 'Score action risk and return go/conditional/no-go guidance before execution.',
    };
  }
  ...
  res.json({
    ...
    currency: 'USDC',
    decimals: 6,
    type: 'bittensor-earnpath',
    note: 'End-to-end Bittensor earning path planning with deterministic JSON outputs.',
    models,
  });
});
...
app.get('/v1/models', (_req, res) => {
  res.json({
    ...
    data: getSupportedModels().map((id) => ({
      ...
      owned_by: 'mozart-earnpath',
    })),
  });
});

app.get('/v1/docs', (_req, res) => {
  const pIncomeMap = formatUnits(getModelPricing('earnpath/income-map')!.inputPer1k, 6);
  const pWeeklyPlan = formatUnits(getModelPricing('earnpath/weekly-plan')!.inputPer1k, 6);
  const pRiskCheck = formatUnits(getModelPricing('earnpath/risk-check')!.inputPer1k, 6);

  res.type('text/plain').send(`# Mozart-Earnpath — Agent Instructions

This is NOT a chat/LLM provider. It returns deterministic JSON earning plans for Bittensor roles.

## Models
- earnpath/income-map ($${pIncomeMap})
- earnpath/weekly-plan ($${pWeeklyPlan})
- earnpath/risk-check ($${pRiskCheck})
...
- Outputs are planning artifacts and should be validated before high-risk execution.
`);
});
