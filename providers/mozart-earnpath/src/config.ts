...
export function loadModels(): void {
  ...
  const incomeMapPrice = parseFloat(optionalEnv('PRICE_INCOME_MAP', '0.006'));
  const weeklyPlanPrice = parseFloat(optionalEnv('PRICE_WEEKLY_PLAN', '0.008'));
  const riskCheckPrice = parseFloat(optionalEnv('PRICE_RISK_CHECK', '0.007'));

  const models: Array<[string, number]> = [
    ['earnpath/income-map', incomeMapPrice],
    ['earnpath/weekly-plan', weeklyPlanPrice],
    ['earnpath/risk-check', riskCheckPrice],
  ];
  ...
  console.log(`Loaded ${activeModels.size} earnpath models`);
}
...
export function loadConfig(): ProviderConfig {
  ...
  return {
    ...
    providerName: optionalEnv('PROVIDER_NAME', 'Mozart-Earnpath'),
