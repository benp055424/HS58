...
function pricingDescription(id: string): string {
  switch (id) {
    case 'treasuryops/runway-forecast':
      return 'Forecast liquidity runway from burn, inflows, and liquid balance (flat per request).';
    case 'treasuryops/cost-envelope':
      return 'Build monthly/weekly/daily spend envelopes with risk-adjusted caps (flat per request).';
    case 'treasuryops/reinvestment-plan':
      return 'Produce a staged reinvestment schedule toward a capital goal (flat per request).';
    default:
      return 'Treasury planning output (flat per request).';
  }
}
...
app.get('/v1/pricing', (_req, res) => {
  ...
  for (const id of getSupportedModels()) {
    ...
    models[id] = {
      ...
      outputPer1kTokens: '0',
      description: pricingDescription(id),
    };
  }
  ...
  res.json({
    ...
    currency: 'USDC',
    decimals: 6,
    type: 'bittensor-treasuryops',
    note: 'Treasury runway, cost envelopes, and reinvestment planning for subnet operators.',
    models,
  });
});
...
app.get('/v1/models', (_req, res) => {
  res.json({
    ...
    data: getSupportedModels().map((id) => ({
      ...
      owned_by: 'mozart-treasuryops',
    })),
  });
});

app.get('/v1/docs', (_req, res) => {
  const pRunway = formatUnits(getModelPricing('treasuryops/runway-forecast')!.inputPer1k, 6);
  const pEnvelope = formatUnits(getModelPricing('treasuryops/cost-envelope')!.inputPer1k, 6);
  const pReinvest = formatUnits(getModelPricing('treasuryops/reinvestment-plan')!.inputPer1k, 6);

  res.type('text/plain').send(`# Mozart-TreasuryOps — Agent Instructions

This is NOT a chat/LLM provider. It returns deterministic treasury JSON: runway forecasts, spend envelopes, and reinvestment schedules for subnet operations.

## Models (flat USDC per request — see GET /v1/pricing)
- treasuryops/runway-forecast ($${pRunway})
- treasuryops/cost-envelope ($${pEnvelope})
- treasuryops/reinvestment-plan ($${pReinvest})
...
3. Send ONE user message whose content is JSON matching the model (fields are optional; defaults apply).

## Input hints
- runway-forecast: { "liquidUsd", "monthlyBurnUsd", "monthlyInflowUsd", "targetRunwayMonths", "subnetId" }
- cost-envelope: { "budgetUsd", "horizonMonths", "riskTolerance": "low"|"medium"|"high", "fixedCostShare", "subnetId" }
- reinvestment-plan: { "availableUsd", "reinvestRate", "goalUsd", "horizonMonths", "subnetId" }

## Example
model: "treasuryops/runway-forecast"
messages: [{"role":"user","content":"{\\"liquidUsd\\":50000,\\"monthlyBurnUsd\\":8000,\\"monthlyInflowUsd\\":6200,\\"targetRunwayMonths\\":6}"}]

## Response
Assistant message content is a JSON string with forecasts, envelopes, or schedules plus metadata.

## Pricing
Flat per model per request in USDC (inputPer1kTokens in /v1/pricing). outputPer1kTokens is always 0.

## Notes
- Rate limit: ${config.rateLimitPerMinute} req/min per channel
- Outputs are planning aids; validate against live books and on-chain balances.
`);
});
