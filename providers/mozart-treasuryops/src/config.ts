...
export function loadModels(): void {
  ...
  const runwayForecastPrice = parseFloat(optionalEnv('PRICE_RUNWAY_FORECAST', '0.004'));
  const costEnvelopePrice = parseFloat(optionalEnv('PRICE_COST_ENVELOPE', '0.006'));
  const reinvestmentPlanPrice = parseFloat(optionalEnv('PRICE_REINVESTMENT_PLAN', '0.008'));

  const models: Array<[string, number]> = [
    ['treasuryops/runway-forecast', runwayForecastPrice],
    ['treasuryops/cost-envelope', costEnvelopePrice],
    ['treasuryops/reinvestment-plan', reinvestmentPlanPrice],
  ];
  ...
  console.log(`Loaded ${activeModels.size} treasuryops models`);
}
...
export function loadConfig(): ProviderConfig {
  ...
  return {
    ...
    providerName: optionalEnv('PROVIDER_NAME', 'Mozart-TreasuryOps'),
