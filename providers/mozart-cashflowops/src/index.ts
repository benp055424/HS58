...
app.get('/v1/pricing', (_req, res) => {
  ...
  for (const id of getSupportedModels()) {
    ...
    models[id] = {
      ...
      outputPer1kTokens: '0',
      description: id === 'cashflowops/revenue-scoreboard'
        ? 'Compute provider-level momentum and efficiency scoreboard from voucher traffic.'
        : id === 'cashflowops/reinvest-policy'
          ? 'Generate capital allocation plan across build, marketing, ops, tao_buy, reserve.'
          : 'Propose measurable flywheel experiments and expected lift targets.',
    };
  }
  ...
  res.json({
    ...
    currency: 'USDC',
    decimals: 6,
    type: 'bittensor-cashflowops',
    note: 'Deterministic cashflow and reinvestment optimization outputs for Bittensor operations.',
    models,
  });
});
...
app.get('/v1/models', (_req, res) => {
  res.json({
    ...
    data: getSupportedModels().map((id) => ({
      ...
      owned_by: 'mozart-cashflowops',
    })),
  });
});

app.get('/v1/docs', (_req, res) => {
  const pScoreboard = formatUnits(getModelPricing('cashflowops/revenue-scoreboard')!.inputPer1k, 6);
  const pReinvest = formatUnits(getModelPricing('cashflowops/reinvest-policy')!.inputPer1k, 6);
  const pFlywheel = formatUnits(getModelPricing('cashflowops/flywheel-tuner')!.inputPer1k, 6);

  res.type('text/plain').send(`# Mozart-CashflowOps — Agent Instructions

This is NOT a chat/LLM provider. It returns deterministic JSON for cashflow and reinvestment decisions.

## Models
- cashflowops/revenue-scoreboard ($${pScoreboard})
- cashflowops/reinvest-policy ($${pReinvest})
- cashflowops/flywheel-tuner ($${pFlywheel})

## Input format
Pass one user message containing JSON with traffic_data.providers entries:
provider, vouchers, active_channels, est_earned_min_usd, est_earned_max_usd.

## Notes
- Rate limit: ${config.rateLimitPerMinute} req/min per channel
- Outputs are deterministic operating plans; validate assumptions before moving capital.
`);
});
