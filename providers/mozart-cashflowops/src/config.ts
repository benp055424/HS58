...
export function loadModels(): void {
  ...
  const revenueScoreboardPrice = parseFloat(optionalEnv('PRICE_REVENUE_SCOREBOARD', '0.009'));
  const reinvestPolicyPrice = parseFloat(optionalEnv('PRICE_REINVEST_POLICY', '0.012'));
  const flywheelTunerPrice = parseFloat(optionalEnv('PRICE_FLYWHEEL_TUNER', '0.014'));

  const models: Array<[string, number]> = [
    ['cashflowops/revenue-scoreboard', revenueScoreboardPrice],
    ['cashflowops/reinvest-policy', reinvestPolicyPrice],
    ['cashflowops/flywheel-tuner', flywheelTunerPrice],
  ];
  ...
  console.log(`Loaded ${activeModels.size} cashflowops models`);
}
...
export function loadConfig(): ProviderConfig {
  ...
  return {
    ...
    providerName: optionalEnv('PROVIDER_NAME', 'Mozart-CashflowOps'),
