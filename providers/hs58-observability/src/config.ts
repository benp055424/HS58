...
export function loadModels(): void {
  ...
  const statusPrice = parseFloat(optionalEnv('PRICE_PROVIDER_STATUS', '0.004'));
  const rankingPrice = parseFloat(optionalEnv('PRICE_PROVIDER_RANKING', '0.006'));
  const routePlanPrice = parseFloat(optionalEnv('PRICE_ROUTE_PLAN', '0.008'));

  const models: Array<[string, number]> = [
    ['observability/provider-status', statusPrice],
    ['observability/provider-ranking', rankingPrice],
    ['observability/route-plan', routePlanPrice],
  ];
  ...
  console.log(`Loaded ${activeModels.size} observability models`);
}
...
export function loadConfig(): ProviderConfig {
  ...
  return {
    ...
    maxResults: parseInt(optionalEnv('MAX_RESULTS', '20')),
