...
app.get('/v1/pricing', (_req, res) => {
  ...
  for (const id of getSupportedModels()) {
    ...
    models[id] = {
      ...
      outputPer1kTokens: '0',
      description: id === 'slaops/sla-guardrail'
        ? 'Check provider fleet against SLA guardrails for latency, quality, and uptime.'
        : id === 'slaops/breach-predict'
          ? 'Predict near-term SLA breach risk under current provider conditions.'
          : 'Generate remediation plan and ownership checklist for SLO recovery.',
    };
  }
  ...
  res.json({
    ...
    currency: 'USDC',
    decimals: 6,
    type: 'bittensor-slaops',
    note: 'SLA reliability and breach prevention intelligence for HS58.',
    models,
  });
});
...
app.get('/v1/models', (_req, res) => {
  res.json({
    ...
    data: getSupportedModels().map((id) => ({
      ...
      owned_by: 'mozart-slaops',
    })),
  });
});

app.get('/v1/docs', (_req, res) => {
  const pGuard = formatUnits(getModelPricing('slaops/sla-guardrail')!.inputPer1k, 6);
  const pPredict = formatUnits(getModelPricing('slaops/breach-predict')!.inputPer1k, 6);
  const pRemediate = formatUnits(getModelPricing('slaops/remediation-plan')!.inputPer1k, 6);

  res.type('text/plain').send(`# Mozart-SlaOps — Agent Instructions

This is NOT a chat/LLM provider. It returns SLA guardrail checks, breach predictions, and remediation plans using live Handshake58 metadata.

## Models
- slaops/sla-guardrail ($${pGuard})
- slaops/breach-predict ($${pPredict})
- slaops/remediation-plan ($${pRemediate})

## Input format
Pass one user message with JSON fields described for each model in /v1/pricing descriptions.

## Notes
- Rate limit: ${config.rateLimitPerMinute} req/min per channel
- Outputs are operational guidance and should be validated by on-call owners.
`);
});
