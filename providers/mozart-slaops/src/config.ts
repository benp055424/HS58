...
export function loadModels(): void {
  ...
  const slaGuardrailPrice = parseFloat(optionalEnv('PRICE_SLA_GUARDRAIL', '0.004'));
  const breachPredictPrice = parseFloat(optionalEnv('PRICE_BREACH_PREDICT', '0.006'));
  const remediationPlanPrice = parseFloat(optionalEnv('PRICE_REMEDIATION_PLAN', '0.008'));

  const models: Array<[string, number]> = [
    ['slaops/sla-guardrail', slaGuardrailPrice],
    ['slaops/breach-predict', breachPredictPrice],
    ['slaops/remediation-plan', remediationPlanPrice],
  ];
  ...
  console.log(`Loaded ${activeModels.size} slaops models`);
}
...
export function loadConfig(): ProviderConfig {
  ...
  return {
    ...
    providerName: optionalEnv('PROVIDER_NAME', 'Mozart-SlaOps'),
