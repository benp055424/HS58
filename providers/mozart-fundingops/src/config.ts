...
export function loadModels(): void {
  ...
  const capitalPlanPrice = parseFloat(optionalEnv('PRICE_CAPITAL_PLAN', '0.004'));
  const leveragePolicyPrice = parseFloat(optionalEnv('PRICE_LEVERAGE_POLICY', '0.006'));
  const liquidityBufferPrice = parseFloat(optionalEnv('PRICE_LIQUIDITY_BUFFER', '0.008'));

  const models: Array<[string, number]> = [
    ['fundingops/capital-plan', capitalPlanPrice],
    ['fundingops/leverage-policy', leveragePolicyPrice],
    ['fundingops/liquidity-buffer', liquidityBufferPrice],
  ];
  ...
  console.log(`Loaded ${activeModels.size} fundingops models`);
}
...
export function loadConfig(): ProviderConfig {
  ...
  return {
    ...
    providerName: optionalEnv('PROVIDER_NAME', 'Mozart-FundingOps'),
