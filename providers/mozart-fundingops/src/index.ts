...
app.get('/v1/pricing', (_req, res) => {
  ...
  for (const id of getSupportedModels()) {
    ...
    models[id] = {
      ...
      outputPer1kTokens: '0',
      description:
        id === 'fundingops/capital-plan'
          ? 'Build a deterministic capital plan: runway, liquidity targets, and funding priority from burn, revenue, and capex inputs.'
          : id === 'fundingops/leverage-policy'
            ? 'Return leverage limits, effective cap, and policy stance from debt, equity, and volatility inputs.'
            : 'Size operating and payout-lag liquidity buffers with gap analysis versus target days of cover.',
    };
  }
  ...
  res.json({
    ...
    currency: 'USDC',
    decimals: 6,
    type: 'bittensor-fundingops',
    note: 'Capital, leverage, and liquidity intelligence for subnet treasury and ops funding.',
    models,
  });
});
...
app.get('/v1/models', (_req, res) => {
  res.json({
    ...
    data: getSupportedModels().map((id) => ({
      ...
      owned_by: 'mozart-fundingops',
    })),
  });
});

app.get('/v1/docs', (_req, res) => {
  const pCapital = formatUnits(getModelPricing('fundingops/capital-plan')!.inputPer1k, 6);
  const pLeverage = formatUnits(getModelPricing('fundingops/leverage-policy')!.inputPer1k, 6);
  const pLiquidity = formatUnits(getModelPricing('fundingops/liquidity-buffer')!.inputPer1k, 6);

  res.type('text/plain').send(`# Mozart-FundingOps — Agent Instructions

This is NOT a chat/LLM provider. It returns deterministic JSON for capital planning, leverage policy, and liquidity buffers for subnet treasury and operations.

## Models
- fundingops/capital-plan ($${pCapital})
- fundingops/leverage-policy ($${pLeverage})
- fundingops/liquidity-buffer ($${pLiquidity})

## How to use via DRAIN
1. Open a payment channel to this provider (drain_open_channel).
2. Call drain_chat with one of the model IDs above and a single user message whose content is JSON (see fields below). Invalid JSON is treated as empty input; defaults apply.

## Input fields (all optional unless noted)
**fundingops/capital-plan:** subnetId, role (miner|validator), monthlyBurnUsd, monthlyRevenueUsd, cashOnHandUsd, targetRunwayMonths, plannedCapexUsd, riskTolerance (conservative|balanced|aggressive).

**fundingops/leverage-policy:** subnetId, role, currentDebtUsd, equityUsd, maxAcceptableLeverage, revenueVolatility (0–1), collateralHaircut (0–1).

**fundingops/liquidity-buffer:** subnetId, role, weeklyOperatingUsd, weeklyInflowUsd, currentLiquidUsd, targetDaysCover, payoutLagDays.

## Example
model: "fundingops/capital-plan"
messages: [{"role":"user","content":"{\\"monthlyBurnUsd\\":800,\\"monthlyRevenueUsd\\":600,\\"cashOnHandUsd\\":4000,\\"targetRunwayMonths\\":6}"}]

## Response
Assistant message content is a JSON string: structured plan metrics, recommendations, and ISO timestamps.

## Pricing
Flat USDC per request; see /v1/pricing for current rates (inputPer1kTokens is the per-call price).

## Notes
- Rate limit: ${config.rateLimitPerMinute} req/min per channel.
- Outputs are planning aids; validate against live books, legal constraints, and exchange/custody policies.
`);
});
