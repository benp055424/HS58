...
app.get('/v1/pricing', (_req, res) => {
  ...
  for (const id of getSupportedModels()) {
    ...
    models[id] = {
      ...
      outputPer1kTokens: '0',
      description: id === 'qualityops/listing-score'
        ? 'Score listing quality from profile completeness, model depth, and latency signals.'
        : id === 'qualityops/trust-check'
          ? 'Check trust posture and enumerate risk flags for a target provider listing.'
          : 'Produce launch gate decision with readiness score and checklist guidance.',
    };
  }
  ...
  res.json({
    ...
    currency: 'USDC',
    decimals: 6,
    type: 'bittensor-qualityops',
    note: 'Provider listing quality intelligence for Handshake58 operators.',
    models,
  });
});
...
app.get('/v1/models', (_req, res) => {
  res.json({
    ...
    data: getSupportedModels().map((id) => ({
      ...
      owned_by: 'mozart-qualityops',
    })),
  });
});

app.get('/v1/docs', (_req, res) => {
  const pListing = formatUnits(getModelPricing('qualityops/listing-score')!.inputPer1k, 6);
  const pTrust = formatUnits(getModelPricing('qualityops/trust-check')!.inputPer1k, 6);
  const pRelease = formatUnits(getModelPricing('qualityops/release-gate')!.inputPer1k, 6);

  res.type('text/plain').send(`# Mozart-QualityOps — Agent Instructions

This is NOT a chat/LLM provider. It returns quality scoring, trust checks, and release gate guidance for Handshake58 listings.

## Models
- qualityops/listing-score ($${pListing})
- qualityops/trust-check ($${pTrust})
- qualityops/release-gate ($${pRelease})
...
### qualityops/listing-score
{
  "providerName": "Mozart-QualityOps",         // required exact/partial provider name
  "modelHint": "openai",                        // optional model hint
  "minScore": 0.7,                                // optional threshold [0..1]
  "maxProviders": 8,                              // optional cap [1..25]
  "marketplaceUrl": "https://handshake58.com"   // optional override
}

### qualityops/trust-check
{
  "providerName": "Mozart-QualityOps",         // required exact/partial provider name
  "minQualityScore": 60,                          // optional quality floor
  "maxMatches": 5,                                // optional cap [1..20]
  "marketplaceUrl": "https://handshake58.com"   // optional override
}

### qualityops/release-gate
{
  "candidateName": "Mozart-NewProvider",        // required candidate provider label
  "category": "data",                            // optional peer category filter
  "targetProtocol": "drain",                     // optional: drain | mpp | x402 | all
  "minBenchmarkCount": 5,                          // optional [1..30]
  "marketplaceUrl": "https://handshake58.com"   // optional override
}

## Response
Assistant content is JSON string with scores, flags, gate decision, and benchmark references.
...
- Use outputs as operational guidance and validate before production decisions.
`);
});
