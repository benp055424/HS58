...
export function loadModels(): void {
  ...
  const gapMapPrice = parseFloat(optionalEnv('PRICE_GAP_MAP', '0.004'));
  const winPlanPrice = parseFloat(optionalEnv('PRICE_WIN_PLAN', '0.006'));
  const defensePlanPrice = parseFloat(optionalEnv('PRICE_DEFENSE_PLAN', '0.008'));

  const models: Array<[string, number]> = [
    ['competeops/gap-map', gapMapPrice],
    ['competeops/win-plan', winPlanPrice],
    ['competeops/defense-plan', defensePlanPrice],
  ];
  ...
  console.log(`Loaded ${activeModels.size} competeops models`);
}
...
export function loadConfig(): ProviderConfig {
  ...
  return {
    ...
    providerName: optionalEnv('PROVIDER_NAME', 'Mozart-CompeteOps'),
