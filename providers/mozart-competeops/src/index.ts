...
app.get('/v1/pricing', (_req, res) => {
  ...
  for (const id of getSupportedModels()) {
    ...
    models[id] = {
      ...
      outputPer1kTokens: '0',
      description:
        id === 'competeops/gap-map'
          ? 'Map capability gaps versus top competitor across chosen dimensions.'
          : id === 'competeops/win-plan'
            ? 'Ranked win moves with suggested spend given resources and timeframe.'
            : 'Defense plan: threat severity, budget split, and moat reinforcement steps.',
    };
  }
  ...
  res.json({
    ...
    currency: 'USDC',
    decimals: 6,
    type: 'bittensor-competeops',
    note: 'Competitive intelligence: gaps, win plans, and defense for subnet operators.',
    models,
  });
});
...
app.get('/v1/models', (_req, res) => {
  res.json({
    ...
    data: getSupportedModels().map((id) => ({
      ...
      owned_by: 'mozart-competeops',
    })),
  });
});

app.get('/v1/docs', (_req, res) => {
  const pGap = formatUnits(getModelPricing('competeops/gap-map')!.inputPer1k, 6);
  const pWin = formatUnits(getModelPricing('competeops/win-plan')!.inputPer1k, 6);
  const pDefense = formatUnits(getModelPricing('competeops/defense-plan')!.inputPer1k, 6);

  res.type('text/plain').send(`# Mozart-CompeteOps — Agent Instructions

This is NOT a chat/LLM provider. It returns deterministic JSON for competitive gap analysis, win plans, and defense plans.

## How to use via DRAIN
1. Open a payment channel to this provider (drain_open_channel).
2. Call drain_chat with the model ID and one user message whose content is JSON (see below).

## Models and flat USDC pricing (per request)
- competeops/gap-map — ${pGap} USDC — score gaps vs a top competitor on multiple dimensions
- competeops/win-plan — ${pWin} USDC — ranked moves and suggested spend to win share
- competeops/defense-plan — ${pDefense} USDC — mitigate threats and reinforce moat under budget

Full numeric rates: GET /v1/pricing

## Input (user message content = JSON)
- competeops/gap-map: optional subnetId, ourCapabilityScore, topCompetitorScore, dimensions (string array), horizonWeeks
- competeops/win-plan: optional subnetId, objective, resourcesUsd, timeframeDays, riskTolerance (low|medium|high)
- competeops/defense-plan: optional subnetId, threats (string array), currentMoatScore (0-1), budgetUsd, slaTargetUptime (0-1)

## Example
model: "competeops/gap-map"
messages: [{"role":"user","content":"{\\"subnetId\\":7,\\"ourCapabilityScore\\":70,\\"topCompetitorScore\\":82,\\"horizonWeeks\\":12}"}]

## Response
Assistant message content is a JSON string: gap rows, ranked win moves, or defense actions.

## Notes
- Rate limit: ${config.rateLimitPerMinute} requests/minute per channel.
- Validate outputs against live market and subnet data before acting.
`);
});
