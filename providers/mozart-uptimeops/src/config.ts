...
export function loadModels(): void {
  ...
  const failureRiskPrice = parseFloat(optionalEnv('PRICE_FAILURE_RISK', '0.004'));
  const incidentPreventionPrice = parseFloat(optionalEnv('PRICE_INCIDENT_PREVENTION', '0.006'));
  const recoveryRunbookPrice = parseFloat(optionalEnv('PRICE_RECOVERY_RUNBOOK', '0.008'));

  const models: Array<[string, number]> = [
    ['uptimeops/failure-risk', failureRiskPrice],
    ['uptimeops/incident-prevention', incidentPreventionPrice],
    ['uptimeops/recovery-runbook', recoveryRunbookPrice],
  ];
  ...
  console.log(`Loaded ${activeModels.size} uptimeops models`);
}
...
export function loadConfig(): ProviderConfig {
  ...
  return {
    ...
    providerName: optionalEnv('PROVIDER_NAME', 'Mozart-UptimeOps'),
