...
function pricingDescription(id: string): string {
  switch (id) {
    case 'uptimeops/failure-risk':
      return 'Estimate operational failure risk from downtime, errors, and dependency shape.';
    case 'uptimeops/incident-prevention':
      return 'Score prevention posture and list guardrails from change and monitoring inputs.';
    case 'uptimeops/recovery-runbook':
      return 'Emit ordered recovery steps and comms templates against RTO/RPO.';
    default:
      return 'Uptime and reliability JSON output.';
  }
}
...
app.get('/v1/pricing', (_req, res) => {
  ...
  res.json({
    ...
    currency: 'USDC',
    decimals: 6,
    type: 'bittensor-uptimeops',
    note: 'Reliability risk, prevention guardrails, and recovery runbooks for subnet operations.',
    models,
  });
});
...
app.get('/v1/models', (_req, res) => {
  res.json({
    ...
    data: getSupportedModels().map((id) => ({
      ...
      owned_by: 'mozart-uptimeops',
    })),
  });
});

app.get('/v1/docs', (_req, res) => {
  const pRisk = formatUnits(getModelPricing('uptimeops/failure-risk')!.inputPer1k, 6);
  const pPrev = formatUnits(getModelPricing('uptimeops/incident-prevention')!.inputPer1k, 6);
  const pRun = formatUnits(getModelPricing('uptimeops/recovery-runbook')!.inputPer1k, 6);

  res.type('text/plain').send(`# Mozart-UptimeOps — Agent Instructions

This is NOT a chat/LLM provider. It returns deterministic JSON for failure risk, incident prevention, and recovery.
...
|----------|-------|---------|
| uptimeops/failure-risk | $${pRisk} | Composite risk index and mitigations |
| uptimeops/incident-prevention | $${pPrev} | Prevention score and guardrails |
| uptimeops/recovery-runbook | $${pRun} | Ordered recovery steps and comms |

Verify live numbers with GET /v1/pricing.

## Input format (user message = JSON)
- failure-risk: optional serviceName, monthlyDowntimeMinutes, errorRate, dependencyCount, criticalPathDepth.
- incident-prevention: optional components[], mtbfHours, changeRatePerWeek, monitoringCoverage.
- recovery-runbook: optional systemName, rtoMinutes, rpoMinutes, lastDrillDaysAgo.

## Example
model: "uptimeops/failure-risk"
messages: [{"role":"user","content":"{\\"serviceName\\":\\"indexer\\",\\"errorRate\\":0.004,\\"dependencyCount\\":8}"}]

## Response
Assistant message content is a JSON string (riskIndex, preventionScore, steps[], etc.).

## Notes
- Rate limit: ${config.rateLimitPerMinute} requests per minute per channel.
- Outputs are planning aids; validate against real SLOs, dashboards, and on-call procedures.
`);
});
